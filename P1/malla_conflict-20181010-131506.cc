...
bool compare ( Tupla3f tuplay , Tupla3f tuplay2 ) {
  return tuplay[1]<tuplay2[1];
}
...
Esfera::Esfera ( const int num_vert_perfil,const int num_instancias_perfil)
{
  ...
  float radio = 1.0;
    ...
    for ( int i = 0 ; i < num_vert_perfil-2; i++){
      ...
      coordenada_x = radio*cos(i);
      coordenada_y = radio*sin(i);
    ...
    for ( int i = 0 ; i < vector_auxiliar.size() ; i++){
      vector_auxiliar[i](0) = abs(vector_auxiliar[i](0));
    }
    
  std::sort (vector_auxiliar.begin(), vector_auxiliar.end(),compare);
